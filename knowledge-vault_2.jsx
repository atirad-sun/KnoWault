...
const STORAGE_KEY = "knowledge-vault-items";
...
export default function KnowledgeVault() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (result?.value) setItems(JSON.parse(result.value));
      } catch {}
      setLoading(false);
    })();
  }, []);
  ...
  const persist = useCallback(async (newItems) => {
    try {
      await window.storage.set(STORAGE_KEY, JSON.stringify(newItems));
    } catch {}
  }, []);
  ...
  if (loading) {
    return (
      <div style={styles.loadingWrap}>
        ...
        <p style={{ color: "#78716C", fontSize: 14, marginTop: 16 }}>Opening your vault…</p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.root}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;0,8..60,700;1,8..60,400&family=DM+Sans:wght@400;500;600&display=swap');
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag-btn:hover { background: #E8E5E1 !important; }
        .cat-btn:hover { background: #F5F5F4 !important; }
        .icon-btn:hover { background: #F5F5F4 !important; }
        .add-btn:hover { transform: scale(1.04); box-shadow: 0 6px 24px rgba(37,99,235,0.25); }
...
const styles = {
  ...
  loadingWrap: {
    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#FAFAF9",
  },
  loadingPulse: {
    width: 40, height: 40, borderRadius: "50%", background: "#2563EB", animation: "pulse 1.5s ease-in-out infinite",
  },
  ...
  smallIconBtn: {
    background: "none", border: "none", fontSize: 16, cursor: "pointer", padding: "2px 6px", borderRadius: 4, transition: "all 0.15s",
  },
  cardTitle: {
